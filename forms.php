...
function ftext($data, $title, $model, $field, $required = false, $type = 'text', $class = '', $placeholder = null) {
  ...
  $content .= '<input id="' . $field . '" type="' . $type . '" name="' . $fieldName . '" value="'
    . (isset($data[$model][$field]) ? $data[$model][$field] : '')
    . '" placeholder="' . $placeholder . '" ' . ($required ? 'required="required" ' : '')
    . ($class != '' ? 'class="' . $class . '" ' : '') . '/>';
  if(isset($data[$model]['_ValidationErrors'][$field])) {
    $content .= '<p class="form-field-error-message">'
      . $data[$model]['_ValidationErrors'][$field] . '</p>';
  }
  $content .= '</div>';
  return $content;
}
...
function fpassword($data, $title, $model, $field, $required = false, $type = 'password') {
  ...
  $content .= '<input id="' . $field . '" type="' . $type . '" name="' . $fieldName . '" value="'
    . (isset($data[$model][$field]) ? $data[$model][$field] : '')
    . '" placeholder="' . $title . '" ' . ($required ? 'required="required" ' : '') . '/>';
  if(isset($data[$model]['_ValidationErrors'][$field])) {
    $content .= '<p class="form-field-error-message">'
      . $data[$model]['_ValidationErrors'][$field] . '</p>';
  }
...
function ftextarea($data, $title, $model, $field, $required = false, $class = '', $placeholder = null) {
  ...
  $content .= '<div class="form-field-group">';
  $content .= '<label for="' . $field . '">' . $title . ($required ? ' <span class="required">*</span>' : '') . '</label>';
  $content .= '<textarea class="' . $class . '" rows="3" id="' . $field . '" name="' . $fieldName
    . '" placeholder="' . $placeholder . '" ' . ($required ? 'required="required" ' : '') . '>';
...
function fselect($data, $title, $model, $field, $options, $required = false, $class = '', $placeholder = null) {
  ...
  $fieldName = 'data[' . $model . '][' . $field . ']';
  $content .= '<label for="' . $field . '">' . $title . ($required ? ' <span class="required">*</span>' : '') . '</label>';
  $content .= '<select ' . ($required ? 'required="required" ' : '') . 'name="' . $fieldName
    . '"' . $defaultSelected . ' id="' . $field . '" class="' . $class . '">';
  $content .= '<option value="">' . $placeholder . '</option>';
  foreach($options as $k => $v) {
    $content .= sprintf('<option %s value="%s">%s</option>',
        (isset($data[$model][$field]) && $data[$model][$field] == $k
          ? ' selected="selected"'
          : ''
        ),
        $k, $v
...
function fcheckbox($data, $model, $options) {
  ...
  foreach($options as $k => $v) {
    ...
    if(isset($data[$model]['_ValidationErrors'][$k])) {
      $content .= '<span class="form-field-error-message checkbox-error">'
        . $data[$model]['_ValidationErrors'][$k] . '</span>';
    }
  }
  $content .= '</ul>';
